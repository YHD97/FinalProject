...
TOTAL_FRAMES = 1000  # Total number of frames to train for
MAX_EPISODE_LENGTH = 18000  # Maximum length of an episode (in frames).  18000 frames / 60 fps = 5 minutes
FRAMES_BETWEEN_EVAL = 100  # Number of frames between evaluations
EVAL_LENGTH = 5  # Number of frames to evaluate for
    ...
    with writer.as_default():
        while frame_number < TOTAL_FRAMES:
            ...
            while epoch_frame < FRAMES_BETWEEN_EVAL:
                ...
                state = np.repeat(process_frame(env.reset()), 1, axis=2)
                life_lost = True
            ...
            if len(rewards) > 20 and SAVE_PATH is not None:
                agent.save(f'{SAVE_PATH}/save-{str(frame_number).zfill(8)}', frame_number=frame_number, rewards=rewards,
                           loss_list=loss_list, player_state=player_list)
