...
class ReplayBuffer:
    ...
    def __init__(self, size=1000, input_shape=(84, 84), history_length=4, use_per=True, input_depth=1):
        """
        Arguments:
        ...
        self.frames = np.empty((self.size, self.input_shape[0], self.input_shape[1], self.input_depth), dtype=np.uint8)
    ...
    def add_experience(self, action, frame, reward, terminal, clip_reward=True):
        ...
        self.frames[self.current, ...] = frame.reshape(self.input_shape[0], self.input_shape[1], self.input_depth)
    ...
    def get_minibatch(self, batch_size=32, priority_scale=0.0):
        ...
        states = np.transpose(np.asarray(states), axes=(0, 1, 2, 3, 4))
        new_states = np.transpose(np.asarray(new_states), axes=(0, 1, 2, 3, 4))
